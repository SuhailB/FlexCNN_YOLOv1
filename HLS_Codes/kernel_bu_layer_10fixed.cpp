...
char project_path[200] = PROJ_DIR;

void print512(bus_t1 burst){
for(int i=0; i<16; i++){
	// u num;
	data_t2 temp = Reinterpret<data_t2>((ap_uint<32>)burst(32*(i+1)-1, 32*i));
	// num.i = ;
	cout<<temp<<"\t";
	// if(i==7) cout<<endl;
	// else cout<<"\t";
	...
	if ((LAYER_IN_H_HW <= IN_H_T + K_T - 1) && (LAYER_IN_W_HW <= IN_W_T + K_T - 1) && !max_pool){
		...
		if (change && FILTER_S == 1){
			...
			for (int hh = 0; hh < LAYER_IN_H_T + FILTER_S - 1; hh++){
				...
				// cout<<"cin_burst_buf["<<local_cin_offset / BUS_PACK_FACTOR0<<"], "<<"global_cin["<<global_cin_offset / BUS_PACK_FACTOR0<<"], "<<sizeof(data_t0) * LAYER_IN_NUM_T * (LAYER_IN_W_T + FILTER_S - 1)<<endl;
			}
		}
		...
		if (INTER_LOAD_EN == 0){
			if ((max_pool && out_num_iter == 0) || separable_conv || conv2d || (UP_SAMPLE_EN && out_num_iter == 0)){
				if (task_cnt == 0){
					...
					// for(int i=0; i<LAYER_IN_NUM_T*((LAYER_IN_H_T+2)*(LAYER_IN_W_T+2))/16; i++){
					// 	print512(cin_burst_buf_ping[i]);
					// }
				} else {
...
void weight_load(
	...
	hls::stream<ConvData0Type>       &fifo_mean_conv,
	hls::stream<ConvData0Type>       &fifo_variance_conv,
	hls::stream<ConvData0Type>       &fifo_scale_conv,
	hls::stream<ConvData0Type>       &fifo_bias_conv,
	hls::stream<ConfigInst>          &fifo_config_out,
	uint layer_id
){
#pragma HLS INLINE off 
	// on-chip buffers
	bus_t1 weight_burst_buf1[IN_NUM_T * K_T * K_T / BUS_PACK_FACTOR1]; //36 512-bit elems, 576 floats
	bus_t1 weight_burst_buf2[OUT_NUM_T * IN_NUM_T * K_T * K_T / BUS_PACK_FACTOR1]; //2304 512-bit elems, 36864 floats
//	bus_t2 bias_burst_buf[OUT_NUM_T / BUS_PACK_FACTOR2];
	bus_t2 beta_depth_burst_buf[IN_NUM_T / BUS_PACK_FACTOR2]; //4 512-bit elems
	bus_t2 gamma_depth_burst_buf[IN_NUM_T / BUS_PACK_FACTOR2];//4 512-bit elems
	bus_t2 beta_conv_burst_buf[OUT_NUM_T / BUS_PACK_FACTOR2]; //4 512-bit elems
	bus_t2 gamma_conv_burst_buf[OUT_NUM_T / BUS_PACK_FACTOR2];//4 512-bit elems	
	bus_t2 mean_conv_burst_buf[OUT_NUM_T / BUS_PACK_FACTOR2]; //4 512-bit elems
	bus_t2 variance_conv_burst_buf[OUT_NUM_T / BUS_PACK_FACTOR2];//4 512-bit elems
	bus_t2 scale_conv_burst_buf[OUT_NUM_T / BUS_PACK_FACTOR2]; //4 512-bit elems
	bus_t2 bias_conv_burst_buf[OUT_NUM_T / BUS_PACK_FACTOR2];//4 512-bit elems
...
#pragma HLS RESOURCE variable=gamma_conv_burst_buf core=XPM_MEMORY uram
    ...
    ap_uint<1>  BATCH_NORM_EN_DEPTH  = LAYER_EN[12];
		...
		uint mean_conv_offset = 0;
		uint variance_conv_offset = 0;
		uint scale_conv_offset = 0;
		uint bias_conv_offset = 0;
		...
		if (norm_depth_en) {
			...
			mean_conv_offset = bias_offset;
			variance_conv_offset = mean_conv_offset + LAYER_OUT_NUM_HW;
			scale_conv_offset = variance_conv_offset + LAYER_OUT_NUM_HW;
			bias_conv_offset = scale_conv_offset + LAYER_OUT_NUM_HW;
		}
        ...
        #pragma HLS pipeline
      	...
      	if (norm_conv_en){
			if (in_num_iter + LAYER_IN_NUM_T >= LAYER_IN_NUM){
				uint global_bias_offset = mean_conv_offset + out_num_iter;
	#ifdef DEBUG_weight
				cout << global_bias_offset << " beta " << beta_conv_offset << endl;
	#endif
				memcpy((void*)mean_conv_burst_buf, (void*)&global_bias[global_bias_offset / BUS_PACK_FACTOR2], sizeof(data_t2) * LAYER_OUT_NUM_T);
			}
			if (in_num_iter + LAYER_IN_NUM_T >= LAYER_IN_NUM){
			uint global_bias_offset = variance_conv_offset + out_num_iter;
	#ifdef DEBUG_weight
			cout << global_bias_offset << " gamma " << gamma_conv_offset << endl;
	#endif
				memcpy((void*)variance_conv_burst_buf, (void*)&global_bias[global_bias_offset / BUS_PACK_FACTOR2], sizeof(data_t2) * LAYER_OUT_NUM_T);
			}
			if (in_num_iter + LAYER_IN_NUM_T >= LAYER_IN_NUM){
				uint global_bias_offset = scale_conv_offset + out_num_iter;
	#ifdef DEBUG_weight
				cout << global_bias_offset << " beta " << beta_conv_offset << endl;
	#endif
				memcpy((void*)scale_conv_burst_buf, (void*)&global_bias[global_bias_offset / BUS_PACK_FACTOR2], sizeof(data_t2) * LAYER_OUT_NUM_T);
			}
			if (in_num_iter + LAYER_IN_NUM_T >= LAYER_IN_NUM){
			uint global_bias_offset = bias_conv_offset + out_num_iter;
	#ifdef DEBUG_weight
			cout << global_bias_offset << " gamma " << gamma_conv_offset << endl;
	#endif
				memcpy((void*)bias_conv_burst_buf, (void*)&global_bias[global_bias_offset / BUS_PACK_FACTOR2], sizeof(data_t2) * LAYER_OUT_NUM_T);
			}
      	}
    }
    ...
    if (bias_en) {
		...
		weight_load_bias_write(mean_conv_burst_buf, fifo_mean_conv, inst0, inst1, inst2, inst3, in_num_iter, out_num_iter);
		weight_load_bias_write(variance_conv_burst_buf, fifo_variance_conv, inst0, inst1, inst2, inst3, in_num_iter, out_num_iter);
		weight_load_bias_write(scale_conv_burst_buf, fifo_scale_conv, inst0, inst1, inst2, inst3, in_num_iter, out_num_iter);
		weight_load_bias_write(bias_conv_burst_buf, fifo_bias_conv, inst0, inst1, inst2, inst3, in_num_iter, out_num_iter);
	}
				...
				for (int w = 0; w < LAYER_IN_W_T / STRIDE; w++){
					...
					for (int i = 0; i < LAYER_IN_NUM_T; i++){
						...
						for (int p = 0; p < FILTER_S; p++)
							conv_kernel_loop1: for (int q = 0; q < FILTER_S; q++){
								...
								fprintf(f,"cout[%d][%d][%d]:%f\tcin[%d][%d][%d]:%f\tweight:\t%f \n", h, w, o, cout_kernel[h][w][o], h * STRIDE + (STRIDE - 1) + p, w * STRIDE + (STRIDE - 1) + q, i, cin[h * STRIDE + (STRIDE - 1) + p][w * STRIDE + (STRIDE - 1) + q][i],  weight[o][p][q][i]);
								#endif
							}
				}
			}
			...
			for (int h = 0; h < LAYER_IN_H_T / STRIDE; h++)
				for (int w = 0; w < LAYER_IN_W_T / STRIDE; w++){
					...
					for (int i = 0; i < LAYER_IN_NUM_T; i++){
						#pragma HLS PIPELINE II=1            
						for (int p = 0; p < FILTER_S; p++)
							conv_kernel_loop2: for (int q = 0; q < FILTER_S; q++){
								cout_kernel[h][w][o] += cin[h * STRIDE + (STRIDE - 1) + p][w * STRIDE + (STRIDE - 1) + q][i] * weight[o][p][q][i];
							}
				}
			}
...
#pragma INLINE off
    ...
    ap_uint<1>  BATCH_NORM_EN_DEPTH  = LAYER_EN[12];
		...
		data_t2 mean_buf[OUT_NUM_T / RELU_LANE][RELU_LANE];
		data_t2 variance_buf[OUT_NUM_T / RELU_LANE][RELU_LANE]; 
		data_t2 scale_buf[OUT_NUM_T / RELU_LANE][RELU_LANE];
		data_t2 bias_buf[OUT_NUM_T / RELU_LANE][RELU_LANE]; 
#pragma HLS ARRAY_PARTITION variable=beta_buf dim=2 complete 
#pragma HLS ARRAY_PARTITION variable=gamma_buf dim=2 complete 
#pragma HLS ARRAY_PARTITION variable=mean_buf dim=2 complete 
#pragma HLS ARRAY_PARTITION variable=variance_buf dim=2 complete 
#pragma HLS ARRAY_PARTITION variable=scale_buf dim=2 complete 
    ...
    bool bias_en = (CONV_EN == 1 && BIAS_EN == 1);
		...
		case 0:
			...
			if (((max_pool || UP_SAMPLE_EN) && out_num_iter == 0) || (!max_pool && (in_num_iter + LAYER_IN_NUM_T >= LAYER_IN_NUM))){
				...
				int w_bound = LAYER_IN_W_T / stride + FILTER_S - 1;
				int h_bound = LAYER_IN_H_T / stride + FILTER_S - 1;
				// cout<<w_bound<<" "<<h_bound<<" "<<LAYER_OUT_NUM_T / RELU_LANE<<endl;
				while(!done1){
#pragma HLS PIPELINE II=1
					ConvData0Type tmp = fifo_cin.read();
					fifo_cout.write(tmp);
		...
		case 1:
		{
			if (in_num_iter + LAYER_IN_NUM_T >= LAYER_IN_NUM){
				...
				for (int o = 0; o < LAYER_OUT_NUM_T / RELU_LANE; o++){
#pragma HLS PIPELINE II=1
					ConvData0Type mean = fifo_mean_conv.read();
					ConvData0Type variance = fifo_variance_conv.read();
					ConvData0Type scale = fifo_scale_conv.read();
					ConvData0Type bias = fifo_bias_conv.read();
					for (int lane = 0; lane < RELU_LANE; lane++){
	#pragma HLS UNROLL
						ap_uint<DATA_W0> u32_mean = mean(DATA_W0 - 1, 0);
						mean_buf[o][lane] = Reinterpret<data_t2>(u32_mean);
						mean = mean >> DATA_W0;
					}
					for (int lane = 0; lane < RELU_LANE; lane++){
	#pragma HLS UNROLL
						ap_uint<DATA_W0> u32_variance = variance(DATA_W0 - 1, 0);
						variance_buf[o][lane] = Reinterpret<data_t2>(u32_variance);
						variance = variance >> DATA_W0;
					}
					for (int lane = 0; lane < RELU_LANE; lane++){
	#pragma HLS UNROLL
						ap_uint<DATA_W0> u32_scale = scale(DATA_W0 - 1, 0);
						scale_buf[o][lane] = Reinterpret<data_t2>(u32_scale);
						scale = scale >> DATA_W0;
					}
					for (int lane = 0; lane < RELU_LANE; lane++){
	#pragma HLS UNROLL
						ap_uint<DATA_W0> u32_bias = bias(DATA_W0 - 1, 0);
						bias_buf[o][lane] = Reinterpret<data_t2>(u32_bias);
						bias = bias >> DATA_W0;
					}
				}
...
#pragma HLS PIPELINE II=1
		...
		case 1:
			if ((max_pool && out_num_iter == 0) || (!max_pool && (in_num_iter + LAYER_IN_NUM_T >= LAYER_IN_NUM))){
				...
				maxpool_w2 <data_t0, OUT_H_T, OUT_W_T, POOL_LANE, 2, DATA_W0> (fifo_cin, fifo_cout, STRIDE, POOL_EN, LAYER_IN_NUM_T, LAYER_IN_H_T, LAYER_IN_W_T, layer_id,0);
			}
			break;
		}
...
#pragma HLS UNROLL
			...
			// data_t2 num[8];
			// for(int i=0; i<8; i++){
			// 	num[i] = Reinterpret<data_t2>((ap_uint<32>)tmp((i+1)*32-1, 32*i));
			// 	printf("%f\t", num[i]);
			// }
			// cout<<"done"<<endl;
			// printf("\n");
			...
			cout_burst_buf[local_cout_idx / BUS_PACK_FACTOR0] = wide_pack;
			// print512(cout_burst_buf[local_cout_idx / BUS_PACK_FACTOR0]);
			// Repeat until the whole tile is read
			w++;
			if (w == LAYER_IN_W_T / 2){
				w = 0;
	...
	if (run){
		...
		case 1:
		{
			for (int hh = 0; hh < LAYER_IN_H_T / 2; hh++){
				...
				// cout<<"global_cout["<<global_cout_idx<<"]"<<"cout_burst_buf["<<local_cout_idx<<"] "<<sizeof(data_t0) * LAYER_IN_W_T / 2 * LAYER_OUT_NUM_T<<endl;
			}
		}
		break;
		case 2:
...
void cout_write(
	...
	bus_t0 cout_burst_buf_ping[OUT_H_T * OUT_W_T * OUT_NUM_T * 3 / 2 / BUS_PACK_FACTOR0] = {0};
	bus_t0 cout_burst_buf_pong[OUT_H_T * OUT_W_T * OUT_NUM_T * 3 / 2 / BUS_PACK_FACTOR0] = {0};
...
#pragma HLS RESOURCE variable=cout_burst_buf_pong core=XPM_MEMORY uram  
	...
	// for(int i=0; i<20; i++){
	// 	if(layer_id==2) print512(cout_burst_buf_ping[i]);
	// }
	// cout<<"--------------------------------"<<endl;
	// We assum that cin has been pre-padded with zeros
	while(!done){
...
#pragma HLS STREAM variable=fifo_gamma_conv depth=16
	...
	hls::stream<ConvData0Type> fifo_scale_conv;
	hls::stream<ConvData0Type> fifo_bias_conv;
#pragma HLS STREAM variable=fifo_mean_conv depth=16
#pragma HLS STREAM variable=fifo_variance_conv depth=16
#pragma HLS STREAM variable=fifo_scale_conv depth=16
#pragma HLS STREAM variable=fifo_bias_conv depth=16
...
// #endif
	...
	// 	FILE *f;
	// 	char dir[100] = PROJ_DIR;
	//  strcat(dir, "data/test/relu_L");	// 	char L[5];
	// 	strcat(dir, itoa(layer_id, L, 10));
	// 	strcat(dir, "_8v1.dat");
	// 	f = fopen(dir, "w");
	...
	// 	FILE *f;
	// 	char dir[100] = PROJ_DIR;
	//  strcat(dir, "data/test/pool_L");
	// 	char L[5];
	// 	strcat(dir, itoa(layer_id, L, 10));
	// 	strcat(dir, "_16x8.dat");
	// 	f = fopen(dir, "w");
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
		...
		cout << "Passed" << layer_id << endl;
			...
			// if((layer_id+1)==2){
			// FILE *f;
			// f = fopen(strcat(project_path, "data/test/L2_out_debug_16x8v1.dat"), "w");
			// for(int j=(4170064)/16; j<(4872288/*7941600+16*(28*4+4)*/)/16; j++){
			// 	// ConvData0Type item = fifo_relu_0.read();
			// 	data_t2 num[16];
			// 	for(int i=0; i<16; i++){
			// 		num[i] = Reinterpret<data_t2>((ap_uint<32>)global_cout[j]((i+1)*32-1, 32*i));
			// 		fprintf(f, "%f\t", num[i]);
			// 	}
			// 	fprintf(f, "\n");
			// }
